...
def getVotesInCountry(countryCode):
  if countries[country].postcode


if doesnt have states or postcodes:
  getVotesPerCountry
  else if has states:
    for each state: getVotesPerState, sum up
    else if has postcodes:
      for each postcode: getVotesPerPostcode, sum up
      VOTES_US

      # of votes per state:
      // 50 times
      select from PostcodeCounter where state = 'NY' - could be more than 1,000 some times
      sum each result.count
      store in memcache
      VOTES_US_NY

      # of votes per postcode: 
      select where name = '90007' and country = 'US'
      VOTES_US_NY_90007
